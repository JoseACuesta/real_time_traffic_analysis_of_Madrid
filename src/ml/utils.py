...
def split_train_validation_and_test_data(
    ...
    if not os.path.exists(train_validation_data_path) or not os.path.exists(
        test_data_path
    ):
...
def split_train_and_validation_data(
    df: pl.DataFrame,
...
def normalize_and_scale_train_and_val_data(
    X_train: pl.DataFrame, X_val: pl.DataFrame
    ...
    logging.basicConfig(
        filename="train_and_evaluate_model.log",
        format="%(asctime)s %(message)s",
        level=logging.INFO,
    )

    categorical_column = [
        col for col, dtype in zip(X_train.columns, X_train.dtypes) if dtype == pl.String
    ]
    numerical_columns = [
        col
        for col, dtype in zip(X_train.columns, X_train.dtypes)
        if dtype in [pl.Int64, pl.Float32]
    ]

    logger.info("columnas numéricas y categóricas separadas")

    X_train_cat = X_train.select(
        categorical_column
    ).to_pandas()  # Para no perder el nombre de las columnas
    ...
    X_train_cat_ohe_df = pl.DataFrame(
        data=X_train_cat_ohe,
        schema=ohe.get_feature_names_out(categorical_column).tolist(),
    )
    logger.info("X_train_cat_ohe_df obtenido")
    X_train_num_scaled_df = pl.DataFrame(
        data=X_train_num_scaled, schema=numerical_columns
    )
    logger.info("X_train_num_scaled_df obtenido")
    X_train_final = pl.concat(
        items=[X_train_cat_ohe_df, X_train_num_scaled_df], how="horizontal"
    )
    logger.info("X_train_final obtenido")

    X_val_cat_ohe_df = pl.DataFrame(
        data=X_val_cat_ohe,
        schema=ohe.get_feature_names_out(categorical_column).tolist(),
    )
    ...
    X_val_final = pl.concat(
        items=[X_val_cat_ohe_df, X_val_num_scaled_df], how="horizontal"
    )
...
def normalize_and_scale_test_data(X_test: pl.DataFrame) -> pl.DataFrame:
    ...
    categorical_column = [
        col for col, dtype in zip(X_test.columns, X_test.dtypes) if dtype == pl.String
    ]
    numerical_columns = [
        col
    ...
    X_test_cat_ohe_df = pl.DataFrame(
        data=X_test_cat_ohe,
        schema=ohe.get_feature_names_out(categorical_column).tolist(),
    )
    logger.info("X_test_cat_ohe_df obtenido")
    X_test_num_scaled_df = pl.DataFrame(
        data=X_test_num_scaled, schema=numerical_columns
    )
    logger.info("X_test_num_scaled_df obtenido")
    X_test_final = pl.concat(
        items=[X_test_cat_ohe_df, X_test_num_scaled_df], how="horizontal"
    )
    logger.info("X_train_final obtenido")
