...
def plot_rfr_data_plotly():
    ...
    total_frames = len(
        [
            f
            for f in os.listdir("data/RandomForestRegressor/file_sink")
            if f.endswith(".csv")
        ]
    )
            ...
            if os.path.exists(path):
                ...
                if is_anomaly:
                    warning_container.warning(
                        body=f"ANOMALY: The value of the load of traffic was supossed to be: {y_test_value}, but {y_pred_value} has been predicted",
                        icon="⚠️",
                    )
                ...
                fig.add_traces(
                    [
                        go.Scatter(
                            ...
                            marker=dict(color=y_pred_colors),
                        ),
                        go.Scatter(
                            x=x_vals_list,
                            y=y_test_list,
                ...
                plotly_chart.plotly_chart(
                    fig, use_container_width=False, key=f"dinamyc_plot_{i}"
                )
                os.remove(path)
                time.sleep(SLEEP)
...
def plot_xgboost_data_plotly():
    ...
    total_frames = len(
        [f for f in os.listdir("data/XGBoost/file_sink") if f.endswith(".csv")]
    )
    ...
    for i in range(total_frames):
        try:
            ...
            if os.path.exists(path):
                ...
                plotly_chart.plotly_chart(
                    fig, use_container_width=True, key=f"dinamyc_plot_{i}"
                )
                os.remove(path)
                time.sleep(SLEEP)
